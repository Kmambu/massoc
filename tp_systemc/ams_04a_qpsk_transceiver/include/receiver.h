...
SC_MODULE(receiver)
{
  ...
  sca_tdf::sca_signal<double> sig_antenna;
  ...
  void set_parameters(double fs, double fc, double fb) {
    int rate = (int)(2*fs/fb);
    i_lna->set_gain(0);
    i_lo->set_carrier(fc);
    i_mixerI->set_gain(20*log10(sqrt(fb)));
    i_integratorI->set_specifications(rate);
    i_samplerI->set_rate(rate);
    i_mixerQ->set_gain(20*log10(sqrt(fb)));
    i_integratorQ->set_specifications(rate);
    i_samplerQ->set_rate(rate);
  }
