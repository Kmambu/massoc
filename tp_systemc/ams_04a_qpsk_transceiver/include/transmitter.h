...
SC_MODULE (transmitter)
{
  ...
  sca_tdf::sca_signal<double> sig_interpolatorI;
  sca_tdf::sca_signal<double> sig_interpolatorQ;
  sca_tdf::sca_signal<double> sig_loI;
  sca_tdf::sca_signal<double> sig_loQ;
  sca_tdf::sca_signal<double> sig_mixerI;
  sca_tdf::sca_signal<double> sig_mixerQ;
  sca_tdf::sca_signal<double> sig_adder;
  sca_tdf::sca_signal<double> sig_pa;
  ...
  void set_parameters(double Eb, double fs, double fc, double fb) {
    int rate = (int)(2.0*fs/fb);
    i_interpolatorI->set_rate(rate);
    i_lo->set_carrier(fc);
    i_mixerI->set_gain(20.0*log10(sqrt(fb)));
    i_interpolatorQ->set_rate(rate);
