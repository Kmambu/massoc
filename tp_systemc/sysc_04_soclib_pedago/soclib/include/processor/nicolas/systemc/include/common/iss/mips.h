 ...
 * Maintainers: nipo
 */
#ifndef SOCLIB_ISS_MIPS_H_
#define SOCLIB_ISS_MIPS_H_
...
{
public:
	static const n_irq = 6;

	void reset();
	bool getIReq( uint32_t &addr );
	bool getDReq( uint32_t &addr, Iss::DataAccess &type, uint32_t &data );
	void setIns( uint32_t ins );
	void setData( uint32_t data );
	void setIrq( uint32_t irqs );
	void setDBerr( Iss::DBerrReason reason );
	void setIBerr( Iss::IBerrReason reason );
	void newCycle();
	void step();
