...
bool Mips::getIReq( uint32_t &addr )
{
	addr = pc;
	return true;
}

bool Mips::getDReq( uint32_t &addr, Iss::DataAccess &type, uint32_t &data )
{
...
void Mips::setData( uint32_t data )
{
	char *type = NULL;
	switch (mem_access) {
	case MEM_LB:
...
void Mips::setIrq( uint32_t irqs )
{
	uint32_t intrs = (irqs << 10) & cp0_reg[CP0_STATUS];
...
void Mips::setDBerr( Iss::DBerrReason reason )
{
	cant_run_code = 'I';
	except = X_IBE;
}

void Mips::setIBerr( Iss::IBerrReason reason )
{
	cant_run_code = 'D';
	except = X_DBE;
...
Mips::Mips( std::string insname, int ident )
	: m_name(insname),
	  m_id(ident)
{
}
