...
int sc_main(int argc, char *argv[]) {
  ...
  const double R1 = 1.0e3;       // Resistance / Ohm
  const double C1 = 1.0 / (2.0 * M_PI * fc);  // Capacitance / F
  ...
  lp_filter_eln lpf_1("lpf_1", R1, C1, H0);
  lpf_1.in(sig_1);
  lpf_1.out(sig_2);
  ...
  // Open trace file.
  sca_trace_file* tfp = sca_create_tabular_trace_file("03_lp_filter_eln_tb");
  ...
  try {
    ...
    tfp->reopen("03_lp_filter_eln_tb_ac");
